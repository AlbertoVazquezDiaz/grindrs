...
const Compatibility = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!isValid) {
            toast.error("No puedes dejar campos vacíos ni elegir el mismo componente dos veces.");
            return;
        }
        ...
        try {
            ...
            console.log("Compatibilidades registradas:", payload);
            ...
            fetchComponentes(); // opcional, para refrescar
        } catch (err) {
    ...
    const columns = [
        { name: "Componente base", selector: (row) => row.componente_base.nombre, sortable: true },
        { name: "Componente compatible", selector: (row) => row.componente_compatible.nombre, sortable: true },
        {
            name: "Acciones",
            cell: (row) => (
    ...
    return (
        <div className="p-6">
            ...
            {showModal && (
                <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-8">
                    <div className="bg-white rounded-lg shadow-xl max-w-3xl w-full relative p-12">
                        ...
                        <h2 className="text-xl font-semibold mb-4">Registrar Compatibilidades</h2>
                        <form
                        ...
                        >
                            {formData.map((item, index) => (
                                <div key={index} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    >
                                        <option value="">Componente base</option>
                                        {componentes.map((c) => (
                                    ...
                                    >
                                        <option value="">Componente compatible</option>
                                        {componentes
            ...
            {isLoading ? (
                ...
                <DataTable
                    columns={columns}
                    data={compatibilidad.filter((c) =>
                        c.componente_base?.nombre?.toLowerCase().includes(search.toLowerCase()) ||
                        c.componente_compatible?.nombre?.toLowerCase().includes(search.toLowerCase())
                    )}
                    pagination
                    highlightOnHover
                    striped
